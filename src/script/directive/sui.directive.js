...
.directive("suiTextbox",function(){
    var vm = {
        template:"<div class='sui-textbox'>\
                    <div class='sui-textbox-line sui-noselect'>\
                        <ul>\
                            <li ng-repeat='row in vm.rows'>{{row}}</li>\
                        </ul>\
                    </div>\
                    <div class='sui-textbox-context'>\
                    </div>\
                    <div class='sui-textbox-page'>\
                        <sui-pagination page='vm.page'></sui-pagination>\
                    </div>\
                    <div class='sui-clear'></div>\
                 <div>"
    }
    ...
    return {
        ...
        controller:function($scope){
            ...
            $scope.$watch("pageIndex",function(pageIndex){
                $scope.vm.page = {pageIndex:pageIndex,pageCount:$scope.pageCount};
            });
            $scope.$watch("pageCount",function(pageCount){
                $scope.vm.page = {pageIndex:$scope.pageIndex,pageCount:pageCount};
            });
        },
        link:function($scope,elements,attrs){
            var textbox = elements[0];
            var context = textbox.getElementsByClassName("sui-textbox-context")[0];
            if(context){
                $scope.$watch("text",function(text){
                    context.innerHTML = text.replace(/\r\n/g,"<br />");
                    var rowCount = parseInt(context.clientHeight/28);
                    $scope.vm.rows = [];
                    for(var i=1;i<=rowCount;i++){
                        $scope.vm.rows.push(i);
                    }
                })
            }
        }
